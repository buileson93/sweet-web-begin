...
DECLARE
    ...
    v_total_questions integer := 20; -- User corrected to 20
    v_emp record;
    v_session_id uuid;
    v_time_ms integer;
    ...
    v_qids uuid[];
BEGIN
    -- Get the actual question IDs for this quiz (taking only 20 if there are more, to be safe, 
    -- but usually the quiz structure should match)
    SELECT array_agg(id) INTO v_qids FROM (
        SELECT id FROM public.questions WHERE quiz_id = v_quiz_id LIMIT v_total_questions
    ) as sub;

    -- First, cleanup previous incorrect seed for this specific quiz and department
    DELETE FROM public.results 
    WHERE quiz_id = v_quiz_id 
    AND employee_id IN (SELECT id FROM public.employees WHERE unit_name ILIKE '%Phòng Kỹ thuật%');
    
    DELETE FROM public.exam_sessions 
    WHERE quiz_id = v_quiz_id 
    AND employee_id IN (SELECT id FROM public.employees WHERE unit_name ILIKE '%Phòng Kỹ thuật%');

    -- Insert new correct seed
    FOR v_emp IN 
    ...
    -- Update candidate_quiz_stats manually
