DROP FUNCTION IF EXISTS public.get_detailed_participation_summary(uuid);

CREATE OR REPLACE FUNCTION public.get_detailed_participation_summary(_quiz_id uuid)
 RETURNS TABLE(id uuid, full_name text, unit_name text, phone text, "position" text, status text, attempts bigint, submitted bigint, best_score text, last_submitted_at timestamptz)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
begin
    ...
    with employee_stats as (
        select 
            ...
            (
                select r.passed || ',' || r.score || '/' || r.total || ',' || r.submitted_at
    ...
    translated_stats as (
        select 
            ...
            nullif(split_part(best_res, ',', 3), '')::timestamptz as last_sub
        from employee_stats
    )
