...
export type ExamSettings = {
  ...
  allowSkip: boolean;
  streakBonus: boolean;
  /** Hiển thị hiệu ứng combo (x2, x3...) — độc lập với việc cộng điểm thưởng chuỗi. */
  comboFx: boolean;
...
export type ReviewItem = {
  ...
  correctText: string;
  explanation: string;
  /** Giải thích riêng cho từng phương án, đã hoán vị cùng thứ tự với `options`. */
...
export const QUESTION_COLUMNS =
  "id, question, options, correct_index, image_url, image_alt, option_images, kind, correct_indices, accepted_answers, pairs, correct_order, difficulty, tags, points, explanation, option_explanations, time_limit_seconds, order_index";

export const QUIZ_COLUMNS =
  "id, title, is_active, status, intro_markdown, start_time, end_time, question_count, duration_minutes, shuffle_options, shuffle_questions, pass_percent, room_password, max_attempts, instant_feedback, allow_fifty_fifty, allow_skip, streak_bonus, combo_fx, show_question_map, negative_marking, blueprint, strict_mode";
