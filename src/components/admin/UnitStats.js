...
import { getScoreDistribution, getUnitStats } from "@/lib/unitStats.functions";
export function UnitStats() {
    ...
    return (<AdminSection title="Thống kê theo đơn vị" description={query.isLoading ? "Đang tải..." : `${rows.length} đơn vị có dữ liệu`} toolbar={<Select value={quizId} onValueChange={setQuizId}>
          ...
          <SelectContent>
            ...
            {quizzes.map((q) => (<SelectItem key={q.id} value={q.id}>
                {q.title}
              </SelectItem>))}
          </SelectContent>
        </Select>} actions={<Button variant="outline" className="rounded-full" onClick={exportExcel} disabled={!rows.length}>
          <Download className="size-4"/> Xuất Excel
        </Button>}>
      <QueryState isLoading={query.isLoading} isError={query.isError} error={query.error} isFetching={query.isFetching} onRetry={() => void query.refetch()} isEmpty={rows.length === 0} skeleton={<ListSkeleton rows={5} height="h-12"/>} empty={<EmptyState icon={Inbox} title="Chưa có dữ liệu thống kê" description="Thống kê sẽ xuất hiện sau khi có bài thi hợp lệ được nộp."/>}>
        <div className="grid gap-4 lg:grid-cols-2">
          <div className="card-elevated p-4">
            ...
            <div className="h-[clamp(15rem,45dvh,20rem)] w-full min-w-0">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartRows} margin={{ left: -20, right: 8, top: 8 }}>
                  ...
                  <Tooltip formatter={(v, n) => [`${v}%`, n]} contentStyle={{ borderRadius: 12, border: "1px solid var(--border)", background: "var(--card)" }}/>
            ...
            {rows.length > chartRows.length ? (<p className="type-meta mt-2">Biểu đồ hiển thị 12 đơn vị có điểm cao nhất.</p>) : null}
          </div>
            ...
            <div className="h-[clamp(15rem,45dvh,20rem)] w-full min-w-0">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={distribution} margin={{ left: -20, right: 8, top: 8 }}>
                  ...
                  <Tooltip formatter={(v) => [`${v} lượt`, "Số lượt"]} contentStyle={{ borderRadius: 12, border: "1px solid var(--border)", background: "var(--card)" }}/>
                  <Bar dataKey="count" name="Số lượt" radius={[6, 6, 0, 0]}>
                    {distribution.map((d) => (<Cell key={d.range} fill={d.fail ? "var(--destructive)" : "var(--primary)"}/>))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            </div>
        ...
        <div className="card-elevated mt-4 overflow-x-auto">
          <table className="w-full min-w-[720px] text-sm">
            ...
            <tbody>
              {rows.map((r) => (<tr key={r.unit} className="border-t border-border transition-colors hover:bg-secondary/40">
