...
export function EmployeeHistoryManager() {
  ...
  const people = useMemo(() => {
    ...
    return Array.from(map.values()).sort((a, b) => b.bestPercent - a.bestPercent || b.attempts - a.attempts);
  }, [resultsQuery.data]);

  const rows = useMemo(() => {
    const kw = normalizeKey(keyword);
    if (!kw) return people;
    return people.filter((p) => normalizeKey(p.name).includes(kw) || normalizeKey(p.unit).includes(kw));
  }, [people, keyword]);
  ...
  return (
    <>
      <AdminSection
        ...
        description={
          resultsQuery.isLoading ? "Đang tải..." : `${rows.length} nhân viên đã tham gia · nhấn vào một dòng để xem chi tiết`
        }
        toolbar={
        ...
        <QueryState
          ...
          empty={
            <EmptyState
              ...
              description={
                keyword ? "Thử từ khoá khác." : "Khi nhân viên hoàn thành bài thi, thành tích sẽ xuất hiện tại đây."
              }
            />
          }
        >
            ...
            {rows.map((p, i) => (
              ...
              >
                ...
                <span className="min-w-0 flex-1">
                  ...
                  <span className="type-meta block truncate">{p.unit || "Chưa cập nhật đơn vị"}</span>
                </span>
                <span className="type-meta hidden sm:block">{p.attempts} lượt</span>
                <span
                  className={cn(
                    "status-pill",
                    p.passed > 0 ? "bg-success/15 text-success" : "bg-warning/15 text-warning-foreground",
                  )}
                >
                  {p.passed > 0 ? `Đạt ${p.passed} cuộc thi` : "Chưa đạt"}
                </span>
                <span className="w-16 text-right font-mono text-sm font-bold text-primary">{p.bestPercent}%</span>
                <span className="type-meta hidden w-40 text-right md:block">{formatDateTime(p.lastAt)}</span>
              </button>
            ))}
          </div>
        </QueryState>
      ...
      <Dialog open={Boolean(selected)} onOpenChange={(o) => !o && setSelected(null)}>
        <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              {selected?.unit || "Chưa cập nhật đơn vị"} · {selected?.attempts} lượt thi · điểm cao nhất{" "}
              {selected?.bestPercent}%
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="flex flex-wrap justify-end gap-2">
            <Button
              variant="outline"
              className="rounded-full"
              onClick={() => selected && void exportData("csv", detailRows(selected), `lich-su-${normalizeKey(selected.name).replace(/ /g, "-")}`)}
            >
              <Download className="size-4" /> CSV
            </Button>
            <Button
              className="rounded-full"
              onClick={() => selected && void exportData("xlsx", detailRows(selected), `lich-su-${normalizeKey(selected.name).replace(/ /g, "-")}`)}
            >
              <FileSpreadsheet className="size-4" /> Excel
            </Button>
          </div>
