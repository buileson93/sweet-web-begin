import { SKILLS, SKILL_COOLDOWN_ROUNDS, skillCooldownLeft, type SkillId } from "@/lib/arena/skills";
      ...
      {SKILLS.map((s) => {
        ...
        return (
          ...
          <button
            ...
            aria-disabled={!ready}
            aria-pressed={on}
            onClick={() => (ready ? onSelect(on ? null : s.id) : undefined)}
            className={cn(
          ...
          <TooltipContent side="top" className="max-w-64 space-y-1 bg-popover p-3 text-popover-foreground shadow-[var(--shadow-lift)]">
            ...
            <p>{s.description}</p>
            <p className="font-mono text-[11px] text-primary">{s.range} · Hồi {SKILL_COOLDOWN_ROUNDS} lượt</p>
            {cd > 0 ? <p className="text-[11px] text-muted-foreground">Còn {cd} câu nữa mới dùng lại được.</p> : null}
          </TooltipContent>
          </Tooltip>
        );
      })}
